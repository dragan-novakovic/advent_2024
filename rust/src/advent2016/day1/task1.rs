...
pub fn runner() {
    ...
    //  let demo = "R5, L5, R5, R3";
    let mvs = convert_input_to_directions(input);
    let result = do_moving(mvs);
    dbg!(result);
}

pub fn do_moving(mvs: Vec<Move>) -> i32 {
    ...
    for (dir, steps) in mvs {
        ...
        match current_orientation {
            Orientation::Bei => position.bei += steps,
            Orientation::Nan => position.nan += steps,
            Orientation::Xi => position.xi += steps,
            Orientation::Dong => position.dong += steps,
        }
    }

    (position.bei - position.nan).abs() + (position.dong - position.xi).abs()
}
